...
const ProblemDescription = ({ problem, _solved }: ProblemDescriptionProps) => {
	...
	const handleLikeClick = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			const { problemDoc, userDoc, problemRef, userRef } = await getUserAndProblemData(transaction);
	...
	const handleDislikeClick = async () => {
		...
		setUpdating(true);
		await runTransaction(firestore, async (transaction) => {
			const { problemDoc, problemRef, userDoc, userRef } = await getUserAndProblemData(transaction);
							...
							{problem?.examples?.map((example, index) => {
								return (
									<div key={example.id}>
										...
										{example.img && <img src={example.img} className="mt-3" alt="" />}
